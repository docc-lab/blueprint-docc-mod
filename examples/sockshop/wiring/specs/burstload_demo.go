...
import (
	...
	"github.com/blueprint-uservices/blueprint/plugins/burstload"
...
// A wiring spec that demonstrates burst load generation in the SockShop application.
// Each service is deployed in its own Docker container with gRPC communication.
// Services generate burst load independently of business logic.
// Burst pattern: Generate requests for a period, then wait, then repeat.
var BurstLoadDemo = cmdbuilder.SpecOption{
	Name:        "burstload_demo",
	Description: "Deploys SockShop services with burst load generation for load testing demonstration",
	Build:       makeSockShopBurstLoadSpec,
}

func makeSockShopBurstLoadSpec(spec wiring.WiringSpec) ([]string, error) {
	// Define the trace collector for monitoring burst load
	trace_collector := zipkin.Collector(spec, "zipkin")

	// Modifiers that will be applied to all services
	applyBurstLoadDefaults := func(serviceName string, useHTTP ...bool) {
		// Add burst load generation - different patterns for different services
		switch serviceName {
		case "payment_service":
			// Payment service: High burst load for stress testing
			burstload.AddBurstLoad(spec, serviceName, 20, "45s", "15s")
		case "user_service":
			// User service: Moderate burst load
			burstload.AddBurstLoad(spec, serviceName, 10, "30s", "20s")
		case "order_service":
			// Order service: Intensive burst load
			burstload.AddBurstLoad(spec, serviceName, 15, "60s", "10s")
		default:
			// Other services: Light burst load
			burstload.AddBurstLoad(spec, serviceName, 5, "20s", "30s")
		}

		// Standard Blueprint plugins
	...
	applyBurstLoadDefaults(shipping_service)
	...
	applyBurstLoadDefaults(order_service)
	...
	applyBurstLoadDefaults(catalogue_service)
	...
	applyBurstLoadDefaults(frontend_service, true) // Only the frontend gets deployed with HTTP
