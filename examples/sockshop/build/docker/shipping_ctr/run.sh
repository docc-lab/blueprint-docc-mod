...
usage() { 
	...
	if [ -z "${OTELCOL_DIAL_ADDR+x}" ]; then
		echo "    OTELCOL_DIAL_ADDR (missing)"
	else
		echo "    OTELCOL_DIAL_ADDR=$OTELCOL_DIAL_ADDR"
	fi
...
shipping_proc() {
	...
	run_shipping_proc() {
        ...
        cd shipping_proc
        ./shipping_proc --shipping_db.dial_addr=$SHIPPING_DB_DIAL_ADDR --otelcol.dial_addr=$OTELCOL_DIAL_ADDR --shipping_service.grpc.bind_addr=$SHIPPING_SERVICE_GRPC_BIND_ADDR --shipping_service.grpc.dial_addr=$SHIPPING_SERVICE_GRPC_DIAL_ADDR &
        SHIPPING_PROC=$!
        return $?
