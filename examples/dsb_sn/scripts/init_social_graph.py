...
async def upload_follow(session, addr, user_0, user_1):
  # Follow(ctx, username string, followeeName string, userId int64, followeeID int64)
  params = {'username': 'username_' + user_0,
            'followeeName': 'username_' + user_1,
            'userId': json.dumps(0),  # Use 0 for username-based lookup
            'followeeID': json.dumps(0)}
  async with session.post(addr + '/Follow', params=params) as resp:
    text = await resp.text()
    if resp.status == 200:
      return 'Success'
    return f'{resp.status} {resp.reason}: {text}'


async def upload_register(session, addr, user):
  # Register(ctx, firstName string, lastName string, username string, password string, userId int64)
  params = {'firstName': 'first_name_' + user,
            'lastName': 'last_name_' + user,
            'username': 'username_' + user,
            'password': 'password_' + user,
            'userId': json.dumps(int(user))}
  async with session.post(addr + '/Register', params=params) as resp:
    text = await resp.text()
    if resp.status == 200:
      return 'Success'
    return f'{resp.status} {resp.reason}: {text}'
...
async def upload_compose(session, addr, user_id, num_users):
  # ComposePost(ctx, userId int64, username string, post_type int64, text string, media_types []string, media_ids []int64)
  ...
  params = {'userId': json.dumps(user_id),
            'username': 'username_' + str(user_id),
            'post_type': json.dumps(0),
            'text': text,
            'media_types': json.dumps(media_types),
            'media_ids': json.dumps(media_ids)}
  async with session.post(addr + '/ComposePost', params=params) as resp:
    text = await resp.text()
    if resp.status == 200:
      return 'Success'
    return f'{resp.status} {resp.reason}: {text}'
...
def printResults(results):
  ...
  for result_type, count in result_type_count.items():
    # Check if it's an HTTP error response (starts with status code)
    if result_type.startswith(('200', '201', '202', '204')):
      print('Succeeded:', count)
    elif result_type == '' or result_type == '{}' or result_type.startswith('Success'):
      print('Succeeded:', count)
    elif '500 Internal Server Error' in result_type or result_type.startswith('500'):
      print('Failed:', count, 'Error:', 'Internal Server Error')
    elif result_type.startswith(('4', '5')):
