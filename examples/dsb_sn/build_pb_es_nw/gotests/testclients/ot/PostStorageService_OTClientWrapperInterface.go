...
type PostStorageService_OTClientWrapperInterface interface {
	ReadPost(ctx context.Context, reqID int64, postID int64) (snnw.Post, error)
	ReadPosts(ctx context.Context, reqID int64, postIDs []int64) ([]snnw.Post, error)
	StorePost(ctx context.Context, reqID int64, post snnw.Post) (error)
...
func (handler *PostStorageService_OTClientWrapper) ReadPost(ctx context.Context, reqID int64, postID int64) (ret0 snnw.Post, err error) {
	...
	if psc := trace.SpanContextFromContext(ctx); psc.IsValid() && !psc.IsSampled() {
		ret0, err = handler.Client.ReadPost(ctx, reqID, postID, "U:"+psc.TraceID().String()+psc.SpanID().String())
		return
	}
...
func (handler *PostStorageService_OTClientWrapper) ReadPosts(ctx context.Context, reqID int64, postIDs []int64) (ret0 []snnw.Post, err error) {
	// Sampling-aware fast path: unsampled parent -> skip all per-request work
	// (baggage/critpath/span/JSON); forward only a compact "U:" carrier so the
	// trace stays consistent. At ratio>=1 nothing is unsampled -> unchanged.
	if psc := trace.SpanContextFromContext(ctx); psc.IsValid() && !psc.IsSampled() {
		ret0, err = handler.Client.ReadPosts(ctx, reqID, postIDs, "U:"+psc.TraceID().String()+psc.SpanID().String())
		return
	}
	// Get baggage from context and create a copy to avoid mutating shared state
	upstreamBaggage := backend.GetBaggageFromContext(ctx)
...
func (handler *PostStorageService_OTClientWrapper) StorePost(ctx context.Context, reqID int64, post snnw.Post) (err error) {
	// Sampling-aware fast path: unsampled parent -> skip all per-request work
	// (baggage/critpath/span/JSON); forward only a compact "U:" carrier so the
	// trace stays consistent. At ratio>=1 nothing is unsampled -> unchanged.
	if psc := trace.SpanContextFromContext(ctx); psc.IsValid() && !psc.IsSampled() {
		err = handler.Client.StorePost(ctx, reqID, post, "U:"+psc.TraceID().String()+psc.SpanID().String())
		return
	}
	// Get baggage from context and create a copy to avoid mutating shared state
	upstreamBaggage := backend.GetBaggageFromContext(ctx)
