...
func makeDockerSpec(spec wiring.WiringSpec) ([]string, error) {
	...
	jaeger_collector := jaeger.Collector(spec, "jaeger_sb")
	...
	user_cache := redis.Container(spec, "user_cache_sb")
	user_db := mongodb.Container(spec, "user_db_sb")
	// post_cache := memcached.Container(spec, "post_cache")
	post_cache := redis.Container(spec, "post_cache_sb")
	post_db := mongodb.Container(spec, "post_db_sb")
	// social_cache := memcached.Container(spec, "social_cache")
	social_cache := redis.Container(spec, "social_cache_sb")
	social_db := mongodb.Container(spec, "social_db_sb")
	urlshorten_db := mongodb.Container(spec, "urlshorten_db_sb")
	// usertimeline_cache := memcached.Container(spec, "usertimeline_cache")
	usertimeline_cache := redis.Container(spec, "usertimeline_cache_sb")
	usertimeline_db := mongodb.Container(spec, "usertimeline_db_sb")
	// hometimeline_cache := memcached.Container(spec, "hometimeline_cache")
	hometimeline_cache := redis.Container(spec, "hometimeline_cache_sb")
	...
	urlshorten_service := workflow.Service[socialnetwork.UrlShortenService](spec, "urlshorten_service_sb", urlshorten_db)
	containers = append(containers, applyDockerDefaults(urlshorten_service))
	// containers = append(containers, "urlshorten_ctr_sb")
	allServices = append(allServices, "urlshorten_service_sb")

	// Define user_mention service
	usermention_service := workflow.Service[socialnetwork.UserMentionService](spec, "usermention_service_sb", user_cache, user_db)
	containers = append(containers, applyDockerDefaults(usermention_service))
	// containers = append(containers, "usermention_ctr_sb")
	allServices = append(allServices, "usermention_service_sb")

	// Define post_storage service
	post_storage_service := workflow.Service[socialnetwork.PostStorageService](spec, "post_storage_service_sb", post_cache, post_db)
	containers = append(containers, applyDockerDefaults(post_storage_service))
	// containers = append(containers, "post_storage_ctr_sb")
	allServices = append(allServices, "post_storage_service_sb")

	// Define media service
	media_service := workflow.Service[socialnetwork.MediaService](spec, "media_service_sb")
	containers = append(containers, applyDockerDefaults(media_service))
	// containers = append(containers, "media_ctr_sb")
	allServices = append(allServices, "media_service_sb")

	// Define uniqueid service
	uniqueId_service := workflow.Service[socialnetwork.UniqueIdService](spec, "uniqueid_service_sb")
	containers = append(containers, applyDockerDefaults(uniqueId_service))
	// containers = append(containers, "uniqueid_ctr_sb")
	allServices = append(allServices, "uniqueid_service_sb")

	// Define user_id service
	userid_service := workflow.Service[socialnetwork.UserIDService](spec, "userid_service_sb", user_cache, user_db)
	containers = append(containers, applyDockerDefaults(userid_service))
	// containers = append(containers, "userid_ctr_sb")
	allServices = append(allServices, "userid_service_sb")

	// Define social_graph service
	socialgraph_service := workflow.Service[socialnetwork.SocialGraphService](spec, "socialgraph_service_sb", social_cache, social_db, userid_service)
	containers = append(containers, applyDockerDefaults(socialgraph_service))
	// containers = append(containers, "socialgraph_ctr_sb")
	allServices = append(allServices, "socialgraph_service_sb")

	// Define home_timeline service
	hometimeline_service := workflow.Service[socialnetwork.HomeTimelineService](spec, "hometimeline_service_sb", hometimeline_cache, post_storage_service, socialgraph_service)
	containers = append(containers, applyDockerDefaults(hometimeline_service))
	// containers = append(containers, "hometimeline_ctr_sb")
	allServices = append(allServices, "hometimeline_service_sb")

	// Define user service
	user_service := workflow.Service[socialnetwork.UserService](spec, "user_service_sb", user_cache, user_db, socialgraph_service, "secret")
	containers = append(containers, applyDockerDefaults(user_service))
	// containers = append(containers, "user_ctr_sb")
	allServices = append(allServices, "user_service_sb")

	// Define text service
	text_service := workflow.Service[socialnetwork.TextService](spec, "text_service_sb", urlshorten_service, usermention_service)
	containers = append(containers, applyDockerDefaults(text_service))
	// containers = append(containers, "text_ctr_sb")
	allServices = append(allServices, "text_service_sb")

	// Define user_timeline service
	usertimeline_service := workflow.Service[socialnetwork.UserTimelineService](spec, "usertimeline_service_sb", usertimeline_cache, usertimeline_db, post_storage_service)
	containers = append(containers, applyDockerDefaults(usertimeline_service))
	// containers = append(containers, "usertimeline_ctr_sb")
	allServices = append(allServices, "usertimeline_service_sb")

	// Define compose post service
	composepost_service := workflow.Service[socialnetwork.ComposePostService](spec, "composepost_service_sb", post_storage_service, usertimeline_service, user_service, uniqueId_service, media_service, text_service, hometimeline_service)
	containers = append(containers, applyDockerDefaults(composepost_service))
	// containers = append(containers, "composepost_ctr_sb")
	allServices = append(allServices, "composepost_service_sb")
	...
	allServices = append(allServices, "wrk2api_service_sb")

	tests := gotests.Test(spec, allServices...)
	containers = append(containers, tests, "otelcol_sb", "jaeger_sb")
