...
func New_HomeTimelineService_OTClientWrapper(ctx context.Context, client HomeTimelineService_OTServerWrapperInterface, coll_client backend.Tracer) (*HomeTimelineService_OTClientWrapper, error) {
	handler := &HomeTimelineService_OTClientWrapper{}
...
func (handler *HomeTimelineService_OTClientWrapper) ReadHomeTimeline(ctx context.Context, reqID int64, userID int64, start int64, stop int64) (ret0 []int64, err error) {
	...
	if psc := trace.SpanContextFromContext(ctx); psc.IsValid() && !psc.IsSampled() {
		ret0, err = handler.Client.ReadHomeTimeline(ctx, reqID, userID, start, stop, "U:"+psc.TraceID().String()+psc.SpanID().String())
		return
	}
...
func (handler *HomeTimelineService_OTClientWrapper) WriteHomeTimeline(ctx context.Context, reqID int64, postID int64, userID int64, timestamp int64, userMentionIDs []int64) (err error) {
	// Sampling-aware fast path: when the enclosing span is head-unsampled, skip
	// ALL per-request instrumentation (baggage copy, span start, JSON marshal)
	// and forward only a compact "U:<traceid><spanid>" carrier so the trace stays
	// consistent (downstream stays unsampled) at ~no cost. At ratio>=1 every span
	// is sampled, so this branch is never taken and behavior is unchanged.
	if psc := trace.SpanContextFromContext(ctx); psc.IsValid() && !psc.IsSampled() {
		err = handler.Client.WriteHomeTimeline(ctx, reqID, postID, userID, timestamp, userMentionIDs, "U:"+psc.TraceID().String()+psc.SpanID().String())
		return
	}
