...
func New_UserIDService_OTClientWrapper(ctx context.Context, client UserIDService_OTServerWrapperInterface, coll_client backend.Tracer) (*UserIDService_OTClientWrapper, error) {
	handler := &UserIDService_OTClientWrapper{}
...
func (handler *UserIDService_OTClientWrapper) GetUserId(ctx context.Context, reqID int64, username string) (ret0 int64, err error) {
	...
	if psc := trace.SpanContextFromContext(ctx); psc.IsValid() && !psc.IsSampled() {
		ret0, err = handler.Client.GetUserId(ctx, reqID, username, "U:"+psc.TraceID().String()+psc.SpanID().String())
		return
	}
