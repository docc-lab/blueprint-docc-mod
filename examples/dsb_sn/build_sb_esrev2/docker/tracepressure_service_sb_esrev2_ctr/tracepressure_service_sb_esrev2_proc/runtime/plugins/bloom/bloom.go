...
// Bytes returns a copy of the raw bit array. Length is ceil(m/8).
// Mirrors bridges/bloom.Filter.ToBytes() for the bit-packed _br wire format.
...
// NewFromBytes rebuilds a Bloom filter from a raw bit array. On size
// mismatch or empty input, returns a fresh empty filter with the
// requested (m, k). Mirrors bridges/bloom.Deserialize.
func NewFromBytes(data []byte, m uint64, k uint) *BloomFilter {
	bf := New(m, k)
	if len(data) == 0 {
		return bf
	}
	if uint64(len(data)) != (m+7)/8 {
		return bf
	}
	copy(bf.bits, data)
	return bf
