...
type MediaService_OTServerWrapperInterface interface {
	ComposeMedia(ctx context.Context, reqID int64, mediaTypes []string, mediaIds []int64, traceCtx string) ([]snnw.Media, error)
}

type MediaService_OTServerWrapper struct {
	Service    snnw.MediaService
	CollClient backend.Tracer
}

func New_MediaService_OTServerWrapper(ctx context.Context, service snnw.MediaService, coll_client backend.Tracer) (*MediaService_OTServerWrapper, error) {
	handler := &MediaService_OTServerWrapper{}
	handler.Service = service
	handler.CollClient = coll_client
	return handler, nil
}

func (handler *MediaService_OTServerWrapper) ComposeMedia(ctx context.Context, reqID int64, mediaTypes []string, mediaIds []int64, traceCtx string) (ret0 []snnw.Media, err error) {
	...
	tr := tp.Tracer("MediaService")
	ctx, span := tr.Start(ctx, "MediaServiceServer_ComposeMedia", trace.WithSpanKind(trace.SpanKindServer))
	defer span.End()
