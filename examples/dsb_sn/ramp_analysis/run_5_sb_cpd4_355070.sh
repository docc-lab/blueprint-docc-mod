#!/bin/bash
echo "=== Rebuild otelcol with cpd=4 + thresholds 35/50/70 ==="
...
run_round() {
  local N=$1
  echo "=== [$(date +%H:%M:%S)] cpd=4 35/50/70 Round $N — teardown all ==="
  ...
  echo "=== [$(date +%H:%M:%S)] cpd=4 35/50/70 Round $N — ramp ==="
  OUT=/users/tomislav/blueprint-docc-mod/examples/dsb_sn/runs/ramp_sb_3tier_cpd4_355070_2to4k_r${N}_$(date +%Y-%m-%d_%H%M%S)
  echo "OUT=$OUT"
  /users/tomislav/blueprint-docc-mod/utils/teardown_seed_ramp.sh sb_esrev0 sb-esrev0 "$OUT"
  echo "=== [$(date +%H:%M:%S)] cpd=4 35/50/70 Round $N done → $OUT ==="
}

for n in 1 2 3 4 5; do
  run_round $n
done
echo "=== [$(date +%H:%M:%S)] ALL 5 cpd=4 35/50/70 ROUNDS DONE ==="
