...
func (handler *UserTimelineService_OTClientWrapper) ReadUserTimeline(ctx context.Context, reqID int64, userID int64, start int64, stop int64) (ret0 []int64, err error) {
	...
	if psc := trace.SpanContextFromContext(ctx); psc.IsValid() && !psc.IsSampled() {
		ret0, err = handler.Client.ReadUserTimeline(ctx, reqID, userID, start, stop, "U:"+psc.TraceID().String()+psc.SpanID().String())
		return
	}
...
func (handler *UserTimelineService_OTClientWrapper) WriteUserTimeline(ctx context.Context, reqID int64, postID int64, userID int64, timestamp int64) (err error) {
	...
	// is sampled, so this branch is never taken and behavior is unchanged.
	if psc := trace.SpanContextFromContext(ctx); psc.IsValid() && !psc.IsSampled() {
		err = handler.Client.WriteUserTimeline(ctx, reqID, postID, userID, timestamp, "U:"+psc.TraceID().String()+psc.SpanID().String())
		return
	}
