...
# This will schedule jaeger to node-3, and all other services to node-2
...
# node-3 -> jaeger; node-2 -> all other sockshop services
kubectl label nodes node-3 workload=jaeger --overwrite || echo "[WARNING] Failed to label node-3"
kubectl label nodes node-2 workload=sockshop --overwrite || echo "[WARNING] Failed to label node-2"
...
node_selectors = {
    'sockshop': {'workload': 'sockshop'},  # All sockshop services on node-2
    'jaeger': {'workload': 'jaeger'},      # Jaeger on node-3
}
...
for yaml_file in deployment_files:
    ...
    if doc and doc.get('kind') == 'Deployment':
        ...
        if 'jaeger' in service_name:
            ...
            print(f"[INFO] Adding node selector to {yaml_file} for node-3 (jaeger)")
        else:
            selector = node_selectors['sockshop']
            print(f"[INFO] Adding node selector to {yaml_file} for node-2 (sockshop)")
...
echo ""
echo "Summary:"
echo "- Jaeger will be scheduled on node-3 (workload=jaeger)"
echo "- All other sockshop services will be scheduled on node-2 (workload=sockshop)"
echo ""
