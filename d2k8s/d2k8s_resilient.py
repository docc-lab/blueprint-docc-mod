...
def update_service_for_daemon(service_content):
    """Update a Service to use internalTrafficPolicy: Local for DaemonSet services."""
    doc = yaml.safe_load(service_content)
    
    if doc.get('kind') == 'Service':
        # Set internalTrafficPolicy to Local to ensure requests only go to pods on the same node
        if 'spec' not in doc:
            doc['spec'] = {}
...
def process_output_files(output_dir, daemon_services=None):
    ...
    for yaml_file in yaml_files:
        ...
        if yaml_file.name.endswith('-deployment.yaml'):
            ...
            if service_name in daemon_services:
                ...
                # Mark this service for later update (after renaming)
                files_to_remove.append(yaml_file)
    ...
    for service_name in daemon_services:
        ...
        if service_file.exists():
            ...
            updated_service_content = update_service_for_daemon(service_content)
            with open(service_file, 'w') as f:
                f.write(updated_service_content)
        else:
            print(f"[WARNING] Service file {service_file} not found for DaemonSet {service_name}")
