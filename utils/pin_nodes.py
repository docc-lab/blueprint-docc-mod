...
def parse_pinning_yaml(path):
    """
    Parse node-pinning YAML into a map: service_name -> { node, requests_cpu?, limits_cpu? }.
    Node names are top-level keys; each value is a list of { service_name: { requests_cpu?, limits_cpu? } }.
    Resources may be omitted (or only comments); then requests_cpu/limits_cpu are None.
    """
...
def apply_pinning_to_deployment(deployment_path, pin_map, dry_run=False):
    ...
    When requests_cpu/limits_cpu are not in the pin config, strip them from the deployment.
    ...
    # Resource requests/limits for all containers (set or strip)
    ...
    for container in containers:
        ...
        if requests_cpu is not None:
            resources.setdefault("requests", {})["cpu"] = requests_cpu
        else:
            resources.pop("requests", None)
        if limits_cpu is not None:
            resources.setdefault("limits", {})["cpu"] = limits_cpu
        else:
...
def main():
    ...
    parser.add_argument(
        "pinning_yaml",
        help="Path to node-pinning YAML (node -> list of service: { requests_cpu?, limits_cpu? })",
    )
    parser.add_argument(
        "k8s_dir",
