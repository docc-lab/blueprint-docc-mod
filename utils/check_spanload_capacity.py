...
def cases():
    ...
    for profile in ("zero", "semconv10-example"):
        ...
        # Tomislav-RetCtx: isolate batching as a possible historical difference;
        # receiver requests remain 512 spans and protobuf/file export is retained.
        result.append(dict(profile=profile, bridge="none", exporter="proto", generators=4,
                           collector_cores=1, rate=4000000 if profile == "zero" else 1000000, collector_batch=8192))
    for case in result:
        ...
        if case.get("collector_batch"):
            case["name"] += f'-batch{case["collector_batch"]}'
    return result
...
def run_case(case, out, seconds):
    assert case["exporter"] == "proto" and case["collector_cores"] == 1
    ...
    config["exporters"]["file"]["format"] = "proto"
    if case.get("collector_batch"):
        config["processors"]["batch"]["send_batch_size"] = case["collector_batch"]
        config["processors"]["batch"]["send_batch_max_size"] = case["collector_batch"]
    if case.get("pprof"):
    ...
    try:
        ...
        row = {**case, "steady_seconds": duration,
               ...
               "exported_spans_per_second": (last["counters"][METRICS[2]] - first["counters"][METRICS[2]]) / duration,
...
def main():
    ...
    write_json(out / "manifest.json", {"created": datetime.now(timezone.utc).isoformat(), "seconds": args.seconds,
        ...
        "note": "Tomislav-RetCtx: one collector, one CPU, 4 GiB; OTLP -> batch -> protobuf file export to /dev/null for every control."})
