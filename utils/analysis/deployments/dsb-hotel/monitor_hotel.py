#!/usr/bin/env python3
"""Continuously observe the deployed hotel; issue three read-only API probes/30s."""
...
ROOT = Path('/users/tomislav/deployments/dsb-hotel/cgpb-es-20260910')
OUT = ROOT / 'monitoring'
NS = 'dsb-hotel'
BASE = 'http://10.10.1.1:21102'
INTERVAL = 30
...
def probe(method, args):
    ...
    if method == 'UserHandler' and result['Ret0'] != 'Login successful':
        raise RuntimeError('Login response unexpected')
        ...
        while not STOP.is_set():
            ...
            try:
                futures = {
                    ...
                    'UserHandler': pool.submit(probe, 'UserHandler',
                        {'username': 'Cornell_1', 'password': '1111111111'}),
                    'SearchHandler': pool.submit(probe, 'SearchHandler',
                        {'customerName': 'Monitoring', 'inDate': '2015-04-09',
                         'outDate': '2015-04-10', 'lat': 37.7835,
                         'lon': -122.41, 'locale': 'en'}),
                    'RecommendHandler': pool.submit(probe, 'RecommendHandler',
                        {'lat': 37.7835, 'lon': -122.41,
                         'require': ('dis', 'rate', 'price')[(cycle - 1) % 3],
                         'locale': 'en'}),
                }
                results = {}
                ...
                if pods:
                    ...
                    futures = {
                        ...
                        'traces': pool.submit(get, f'http://{jaeger}:16686/api/traces?' + urllib.parse.urlencode({
                            'service': 'unknown_service:frontend_service_hotel_cgpb_es_proc',
                            'limit': 5, 'lookback': '1h'})),
                    }
                    if cycle % 2 == 1:
