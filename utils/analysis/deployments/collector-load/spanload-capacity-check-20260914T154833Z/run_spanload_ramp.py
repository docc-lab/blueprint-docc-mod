...
def summarize_step(variant, rate, phase, samples, args):
    ...
    row = {"variant": variant, "profile": getattr(args, "profile", "zero"), "target_spans_per_second": rate,
...
def run_step(variant, rate, directory, collector_pid, grpc_port, metrics_url, fraction, args):
    ...
    argv = ["taskset", "-c", args.generator_cpus, str(args.binary), "--endpoint", f"127.0.0.1:{grpc_port}",
            "--insecure", "--profile", args.profile, "--bridge", variant, "--rates", str(rate),
...
def plot_results(rows, out, profile="zero"):
    ...
    fig.suptitle(f"{label} · 1 collector CPU, 4 GiB · JSON export to /dev/null")
