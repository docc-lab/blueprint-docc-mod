#!/usr/bin/env python3
"""Tomislav-RetCtx: isolate collector capacity with extra senders and cheaper export controls."""
...
GENERATOR_CORES = ["4-7", "14-17"]


def cases():
    # Same aggregate offers as overloaded ramp points; double processes,
    # connections, workers, and available sender cores without changing the collector.
    result = []
    for profile in ("zero", "semconv10-example"):
        for bridge in ("none", "pb", "cgpb", "sb"):
            result.append(dict(profile=profile, bridge=bridge, exporter="json", generators=2,
                               collector_cores=1, rate=(650000 if bridge == "none" else 450000) if profile == "zero" else 150000))
    # Faster collectors establish a measured lower bound on sender capacity.
    # Nop accepts decoded OTLP and runs batch, but does no file serialization.
    for profile in ("zero", "semconv10-example"):
        for bridge in ("none", "sb"):
            result.append(dict(profile=profile, bridge=bridge, exporter="nop", generators=1,
                               collector_cores=4, rate=2000000 if profile == "zero" else 500000))
        result.append(dict(profile=profile, bridge="none", exporter="proto", generators=1,
                           collector_cores=1, rate=2000000 if profile == "zero" else 500000))
    # Profile separately so sampling overhead is not hidden in comparison runs.
    for profile in ("zero", "semconv10-example"):
        result.append(dict(profile=profile, bridge="none", exporter="json", generators=1,
                           collector_cores=1, rate=650000 if profile == "zero" else 150000, pprof=True))
    for case in result:
