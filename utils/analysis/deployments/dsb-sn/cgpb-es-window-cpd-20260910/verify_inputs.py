...
common_count = min(map(len, all_hashes.values()))
assert common_count >= run.SPEC['requests_per_phase'] - 1
assert all_hashes['fixed2'][:common_count] == all_hashes['fixed4'][:common_count] == all_hashes['random2_6'][:common_count], 'Stored request inputs differ'
run.save(run.ROOT / 'input-equivalence.json', {'matched': True,
    'matched_requests_per_phase': common_count, 'all_measured_counts': {name: len(hashes) for name, hashes in all_hashes.items()}, 'lua_seed': 42,
    ...
    'hashes': all_hashes})
print(f'The first {common_count} stored request inputs match across the three phases, in request order; complete per-phase data is retained.')
