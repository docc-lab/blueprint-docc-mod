...
draws = comparison[2]['root_draw_counts']
...
axes[2].set_xlabel('Root-selected distance (window descriptor)')
...
fig.supxlabel(f'{comparison[0]["requests"]} matching Lua ComposePost inputs per setting · 5 requests/sec · 23 spans/request · root → deepest leaf: 7 spans', fontsize=10)
fig.savefig(ROOT / 'comparison.png', dpi=170)
fig.savefig(ROOT / 'comparison.svg')
plt.close(fig)

# Read sizes from exported trusses, rather than plotting a configured maximum.
observed = {}
for trace in analyses['random2_6']['traces']:
    for row in trace['rows']:
        if row['checkpoint']:
            observed.setdefault(row['window_distance'], set()).add(row['bloom_bytes'])
assert all(len(widths) == 1 for widths in observed.values())
fig, ax = plt.subplots(figsize=(8.5, 4.4), layout='constrained')
distances = sorted(observed)
widths = [next(iter(observed[d])) for d in distances]
ax.bar(distances, widths, color='#c76430', width=.65)
ax.axhline(12, color='#64748b', linestyle='--', label='Previous sizing: 12 bytes for every 2–6 window')
for distance, width in zip(distances, widths):
    ax.text(distance, width + .25, f'{width} B', ha='center', fontweight='bold')
ax.set_xticks(distances)
ax.set_ylim(0, 15)
ax.set_xlabel('Intended checkpoint distance, decoded from the truss')
ax.set_ylabel('Bloom filter bytes')
ax.set_title('Bloom sizes now follow each selected window', fontsize=15, pad=16)
ax.legend(loc='upper left', fontsize=9, frameon=False)
fig.savefig(ROOT / 'filter-sizes.png', dpi=170)
fig.savefig(ROOT / 'filter-sizes.svg')
plt.close(fig)
...
print('Created comparison, filter-sizes, and checkpoint-paths PNG/SVG figures from measured traces.')
