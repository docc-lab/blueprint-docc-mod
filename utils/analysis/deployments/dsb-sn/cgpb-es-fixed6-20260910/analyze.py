...
def analyze_trace(trace, phase):
    ...
    nonbranch_records = []
    window_distances = {}
    bloom_byte_counts = Counter()
    bloom_bytes_total = 0
    descriptor_bytes_total = 0
    max_sized_original_payload_bytes = 0
    for sid, span in spans.items():
        d = depth(sid)
        ...
        if key == '_br':
            ...
            bloom_start = offset + 8
            if 'cpd_min' in config:
                distance = raw[bloom_start] + 1
                bloom_start += 1
                descriptor_bytes_total += 1
                assert low <= distance <= high
                capacity = max(1, distance - 1)
                m = math.ceil(-capacity * math.log(0.0001) / math.log(2) ** 2)
                k = math.ceil(m / capacity * math.log(2))
                bloom_bytes = (m + 7) // 8
                window = root if sid == root else anchor
                assert window_distances.setdefault(window, distance) == distance, 'Sibling window descriptors disagree'
            else:
                distance = low
            assert len(raw) >= bloom_start + bloom_bytes
            bloom = raw[bloom_start:bloom_start + bloom_bytes]
            ha = raw[bloom_start + bloom_bytes:]
            row['window_distance'] = distance
            row['bloom_bytes'] = bloom_bytes
            bloom_byte_counts[bloom_bytes] += 1
            bloom_bytes_total += bloom_bytes
            max_m = math.ceil(-max(1, high - 1) * math.log(0.0001) / math.log(2) ** 2)
            max_sized_original_payload_bytes += offset + 8 + (max_m + 7) // 8 + len(ha)
            ...
            if sid != root:
                ...
                assert 1 <= gap <= high
                assert gap <= distance, 'Checkpoint exceeded its intended window distance'
                if sid in interior_checkpoints:
                    assert gap == distance, ('Scheduled checkpoint does not match its window distance', row)
                    gaps[gap] += 1
                else:
                    leaf_gaps[gap] += 1
            offset = 0
            ...
            max_sized_original_payload_bytes += len(raw)
        if low == high:
            assert (sid in checkpoints) == (d % low == 0 or sid in leaves)
        rows.append(row)

    # Window descriptors reveal the intended distance even for early leaves.
    # Legacy fixed-mode controls obtain it from their fixed configuration.
    draws = []
    for anchor, descendants in checkpoint_children.items():
        ...
        candidates = [window_distances[anchor] if 'cpd_min' in config else low]
        assert all(gap <= candidates[0] for _, gap in descendants)
        if complete:
            assert complete == set(candidates)
        assert candidates
    ...
    result = {'trace_id': trace['traceID'], 'root_start_us': spans[root]['startTime'],
              ...
              'bloom_bytes': bloom_bytes_total, 'window_descriptor_bytes': descriptor_bytes_total,
              'bloom_width_counts': dict(bloom_byte_counts),
              'hypothetical_original_max_sized_payload_bytes': max_sized_original_payload_bytes,
...
def analyze_phase(phase, limit=None):
    directory = ROOT / phase['name']
    traces = json.loads((directory / 'traces.json').read_text())
    result = json.loads((directory / 'result.json').read_text())
    all_analyzed = sorted((analyze_trace(t, phase) for t in traces), key=lambda t: t['root_start_us'])
    assert len(all_analyzed) == result['requests']
    assert SPEC['requests_per_phase'] - 1 <= len(all_analyzed) <= SPEC['requests_per_phase']
    analyzed = all_analyzed[:limit] if limit is not None else all_analyzed
    gaps, leaf_gaps, root_draws, widths = Counter(), Counter(), Counter(), Counter()
    for t in analyzed:
        gaps.update(t['scheduled_interior_gaps'])
        leaf_gaps.update(t['leaf_gaps'])
        widths.update(t['bloom_width_counts'])
    ...
    summary = {'phase': phase['name'], 'config': phase['config_map'], 'requests': len(analyzed),
               'all_measured_requests': len(all_analyzed),
               ...
               'bloom_bytes_per_request': statistics.mean(t['bloom_bytes'] for t in analyzed),
               'window_descriptor_bytes_per_request': statistics.mean(t['window_descriptor_bytes'] for t in analyzed),
               'bloom_width_counts': dict(sorted(widths.items())),
               'hypothetical_original_max_sized_payload_bytes_per_request': statistics.mean(t['hypothetical_original_max_sized_payload_bytes'] for t in analyzed),
               ...
               'root_draw_counts': dict(sorted(root_draws.items())),
               ...
               'validated': ['complete parent tree', 'all server leaves checkpointed',
                   ...
                   'Bloom width matches intended distance', 'per-window descriptors agree across siblings',
                   'branch record ancestry and depth', 'scheduled interior gap range',
                   'shared countdown across siblings', 'no error span flags']}
    (directory / 'analysis.json').write_text(json.dumps({'summary': summary, 'traces': analyzed,
        'additional_traces': all_analyzed[len(analyzed):]}, indent=2) + '\n')
    with (directory / 'span-details.csv').open('w') as stream:
        fields = ['trace_id', 'span_id', 'parent_span_id', 'operation', 'depth', 'checkpoint',
                  'leaf', 'anchor_span_id', 'checkpoint_gap', 'window_distance', 'bloom_bytes', 'payload_bytes']
...
def main():
    ...
    common_count = min(json.loads((ROOT / p['name'] / 'result.json').read_text())['requests'] for p in phases)
    results = [analyze_phase(p, common_count) for p in phases]
    if len(results) == 3:
        ...
        print(f'All {common_count} request trees per phase match in request order; every additional trace was also validated.')
