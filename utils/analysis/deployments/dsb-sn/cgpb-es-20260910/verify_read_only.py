"""Verify deployment, timeline reads, and traces; retain the known post-ID failure."""
...
requests = []
# Reuse the three earlier smoke writes, whose duplicate-ID failure remains
# recorded in verification.log. This verification sends timeline reads only.
posts = [1789051571424, 1789051571440, 1789051571440]
compose_traces = json.loads((ROOT / 'compose-smoke-traces.json').read_text())['data']
assert len(compose_traces) == 3
...
print(f'{len(requests)} concurrent timeline reads passed; earlier post-ID uniqueness check FAILED', flush=True)
...
summary = {'verified_at': dt.datetime.now(dt.timezone.utc).isoformat(), 'namespace': NS,
    ...
    'successful_api_requests': len(requests), 'prior_compose_requests': 3, 'prior_compose_post_ids': posts,
    'application_checks_passed': False, 'duplicate_post_id_detected': True,
    'post_id_fix_deployed': False,
