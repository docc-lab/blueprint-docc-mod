#!/usr/bin/env bash
# Tomislav-RetCtx: continuation of hotelnw_n5_chain.sh (user 2026-09-24: "after that let's recompile stuff with the
...
cd $REPO/utils
NTR=$(head -1 $ROOTS)
echo "$(date -u +%H:%M:%S) waiting for the nt runner ($NTR)"
while pgrep -f "[r]un_dsb_sn_nw.py run --out $NTR" >/dev/null; do sleep 15; done
[ -f $NTR/run/01-nt/complete.json ] || fail "nt runner exited without complete.json"
echo "$(date -u +%H:%M:%S) done hotelnw nt"
STAMP=$(date -u +%Y%m%dT%H%M%SZ)
# --- v: passes 2..5 (old images, as its n=1 pass 1)
N1=$(cat $S/hotelnw_v_root.txt); R=/users/tomislav/deployments/dsb-hotel/retctx-hotelnw-n5-v-$STAMP
$PY derive_dsb_sn_nw.py --source $SRC --provenance-from $SRC $COMMON --ramp-passes 4 --first-pass 2 --rates $(seq -s ' ' 10000 1000 33000) --out $R --kinds v --note "HotelReservation ZERO-WORK, passes 2..5 (n=5 with the n=1 sweep as pass 1), 10k..33k, one deployment, 60 s between passes; current best stack. No smoke." > $S/hotelnw_n5_v_derive.log 2>&1 || fail "derive v"
check $R $N1 v 0; echo $R >> $ROOTS
echo "$(date -u +%H:%M:%S) run hotelnw v passes 2..5 10000..33000 $R"
$PY run_dsb_sn_nw.py run --out $R --skip-smoke > $S/hotelnw_n5_v_run.log 2>&1 || fail "run v"
