...
R = Path(open('/tmp/claude-20002/-users-tomislav/6693a286-50fe-4f08-88f4-79a32690db39/scratchpad/esfix2_root').read().strip())
...
# esfix additions: smoke status, export ceiling check per point
for name in ('smoke-status.json', 'smoke-complete.json', 'run-complete.json'):
    p = R / name
    if p.exists(): print(name, p.read_text().strip()[:200])
for case in sorted(p for p in (R / 'run').iterdir() if p.is_dir() and '-interrupted-' not in p.name) if (R / 'run').exists() else []:
    if (case / 'es-template.json').exists(): print('  es-template installed')
    if (case / 'es-index.json').exists():
        st = json.loads((case / 'es-index.json').read_text()); print('  es-index tag fields:', {k: len(v) for k, v in st['tag_fields'].items()})
    print('  offered | accepted/s | exported/s | sdk_drop/s | completed | mean_ms')
    for p in sorted(case.glob('rate-*/result.json')):
        r = json.loads(p.read_text())
        if 'kind' not in r: continue
        d = r['collector_deltas']; s = r['wall_seconds']
        print(f"  {r['offered_rps']:6} | {d.get('otelcol_receiver_accepted_spans_total',0)/s:8.0f} | {d.get('otelcol_exporter_sent_spans_total',0)/s:8.0f} | {r.get('periodic_counters',{}).get('deltas',{}).get('sdk',{}).get('spans_dropped',0)/s if 'periodic_counters' in r else float('nan'):6.0f} | {r['completed_rps']:7.0f} | {r['mean_ms']:8.1f}")
