...
REPO = Path("/users/tomislav/blueprint-docc-mod")  # Tomislav-RetCtx: scratch copy for the GOMAXPROCS test
...
def summarize_step(variant, rate, phase, samples, args):
    ...
    row["capacity_limited"] = (row["collector_cpu_cores"] >= .94 * ARGS_QUOTA[0] and row["exported_spans_per_second"] < rate * .93)
    return row
...
def run_variant(variant, args, fraction):
    ...
    # Tomislav-RetCtx: Kubernetes-like placement for the GOMAXPROCS test: a CFS quota of
    # --collector-quota CPUs, optionally no cpuset (the pod sees every core, as on our nodes),
    # and GOMAXPROCS either explicit or unset (Go 1.24 then uses every visible core).
    argv = ["docker", "run", "-d", "--name", name, "--network", "host"]
    if not args.no_cpuset:
        argv += ["--cpuset-cpus", str(args.collector_cpu)]
    argv += ["--cpus", str(args.collector_quota), "--memory", "4g", "--memory-swap", "4g", "-e", "GOGC=100"]
    if args.collector_gomaxprocs != "unset":
        argv += ["-e", "GOMAXPROCS=" + args.collector_gomaxprocs]
    ...
    try:
        ...
        assert details["HostConfig"]["NanoCpus"] == int(args.collector_quota * 1_000_000_000)
...
def main():
    ...
    parser.add_argument("--collector-quota", type=float, default=1.0)
    parser.add_argument("--collector-gomaxprocs", default="1", help="GOMAXPROCS value, or 'unset'")
    parser.add_argument("--no-cpuset", action="store_true")
    ...
    ARGS_QUOTA[0] = args.collector_quota
