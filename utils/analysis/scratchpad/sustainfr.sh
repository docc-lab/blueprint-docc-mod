#!/usr/bin/env bash
# Tomislav-RetCtx: SUSTAINED-ACCEPTANCE probe, FAST-RETRY collectors (admission6040fr). Fixed-interval arrivals at 3500 / 4000 / 4500 req/s
...
NOTE="Sustained-acceptance probe with fast exporter retry (admission6040fr: retry 200 ms initial / 1 s max instead of 5 s / 30 s; Jaeger queue unchanged): fixed-interval 3500 4000 req/s for 600 s each, v then cgpb (reverse on), collectors admission6040fr (1 CPU, 256Mi, GOMEMLIMIT 230MiB; priority soft 40 / hard 60, vanilla memory_limiter 60/20). Finds the highest rate with zero refusals and no growing exporter backlog over 10 min; the bursty runs are re-centred just below it. Same images (commit f57a8699)."
arm=on
STAMP=$(date -u +%Y%m%dT%H%M%SZ)
R=/users/tomislav/deployments/dsb-sn/retctx-nwe2e-sustainfr-$arm-$STAMP
log "arm $arm -> $R"
sg docker -c "cd $REPO && .venv/bin/python -B -u utils/derive_dsb_sn_nw.py \
    ...
    --collector admission6040fr --reverse $arm --rates 3500 4000 --seconds-per-rate 600 \
    --note '$NOTE'" >>"$LOG" 2>&1 \
  || { log "DERIVE FAILED"; exit 1; }
echo "$R" >> "$S/sustainfr_roots.txt"
