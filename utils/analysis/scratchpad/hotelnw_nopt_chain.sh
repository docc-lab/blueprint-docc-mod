...
if [ -z "$DRYRUN" ]; then
  ...
  # Tomislav-RetCtx (user 2026-09-25 ~03:15Z): the SN no-work span-loss (bursty) experiment runs first
  until grep -q "SNBURST COMPLETE" $S/snburst_chain.log 2>/dev/null; do
    grep -q "SNBURST FAILED" $S/snburst_chain.log 2>/dev/null && fail "span-loss chain failed; not starting"; sleep 30; done
  echo "$(date -u +%H:%M:%S) span-loss chain complete; starting the hotel no-work bridge rerun without passthrough"
fi
