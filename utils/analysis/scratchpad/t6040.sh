...
NOTE="Shedding-threshold probe: identical to the n=5 admission matrix (1 CPU, 256Mi, GOMEMLIMIT 230MiB) except the thresholds. Priority processor soft 40 / hard 60 (92 / 138 MiB) instead of 50 / 70; vanilla memory_limiter 60 instead of 70. Every configuration idles at 70-77 MiB, so these sit above the floor but below the transient peaks that used to gate onset, and each configuration should shed before its knee. Same images (commit f57a8699)."

for arm in on off; do
  if [ "$arm" = "on" ]; then KINDS="nt v pb cgpb sb"; else KINDS="pb cgpb sb"; fi
  STAMP=$(date -u +%Y%m%dT%H%M%SZ)
  R=/users/tomislav/deployments/dsb-sn/retctx-nwe2e-t6040-$arm-$STAMP
  log "arm $arm -> $R"
  sg docker -c "cd $REPO && .venv/bin/python -B -u utils/derive_dsb_sn_nw.py \
      ...
      --collector admission6040 --reverse $arm --note '$NOTE arm=$arm.'" >>"$LOG" 2>&1 \
    || { log "arm $arm: DERIVE FAILED"; exit 1; }
  echo "$R" >> "$S/t6040_roots.txt"
