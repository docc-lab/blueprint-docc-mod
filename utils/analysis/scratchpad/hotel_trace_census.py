...
def roots():
    out = []
    for x in ('hotel_mem_v_root', 'hotel_mem_br_root'):
...
while True:
    for R in roots():
        for case in sorted(glob.glob(f'{R}/run/01-*')):
            ...
            for f in pts:
                ...
                ip = ch_ip(kind)
                if not ip:
                    json.dump({'missed': True, 'reason': 'case no longer deployed'}, open(out, 'w')); print(f"TRACES hotel {kind} {d['offered_rps']}: MISSED (case gone)", flush=True); continue
                ...
                try: rows = census(ip, d)
                    ...
                    print(f"TRACES hotel {kind} {d['offered_rps']}: query error {e.__class__.__name__}: {str(e)[:120]}", flush=True); continue
                ...
                line = (f"TRACES hotel {kind} {d['offered_rps']}: affected {res['affected_pct']:.2f}% of {comp:,} traces"
    ...
    try:
        if 'HOTEL MEM CHAIN COMPLETE' in open(f'{S}/hotel_mem_chain.log').read(): print('TRACE CENSUS: hotel sweep complete', flush=True); break
    except OSError: pass
    time.sleep(10)
