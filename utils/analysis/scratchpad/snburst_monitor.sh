...
n=$(wc -l < $S/snburst_chain.log)
while true; do
  m=$(wc -l < $S/snburst_chain.log)
  [ "$m" -gt "$n" ] && { sed -n "$((n + 1)),${m}p" $S/snburst_chain.log | grep -E "run |done |COMPLETE|FAILED|boundary"; n=$m; }
  grep -q -E "SNBURST (COMPLETE|FAILED)" $S/snburst_chain.log && exit 0
  R=$(tail -1 $S/snburst_roots.txt)
