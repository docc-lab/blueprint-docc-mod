...
roots() { for x in hotelnw_nt_root hotelnw_v_root hotelnw_br_root; do cat $S/$x.txt 2>/dev/null; done; }
...
while true; do
  ...
  for R in $(roots); do
    for f in $(find $R/run/ -mindepth 3 -maxdepth 3 -name result.json 2>/dev/null | sort); do
      ...
      echo $f >> $seen
      python3 $S/stream8_point.py $f $S
    done
  done
  ...
  L=$(tail -1 $S/hotelnw_chain.log 2>/dev/null); [ "$L" != "${PL:-}" ] && { echo "CHAIN: $L" | cut -c1-220; PL=$L; last=$(date +%s); }
  grep -q "HOTELNW CHAIN COMPLETE" $S/hotelnw_chain.log 2>/dev/null && { echo "HOTELNW SWEEP COMPLETE"; exit 0; }
  kill -0 $(cat $S/hotelnw_chain.pid 2>/dev/null || echo 999999) 2>/dev/null || { echo "HOTELNW CHAIN GONE: $(tail -3 $S/hotelnw_chain.log | cut -c1-200)"; exit 1; }
