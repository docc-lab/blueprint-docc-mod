...
# v pb cgpb sb on 1000..20000 step 1000. GOGC=off GOMEMLIMIT=1GiB gctrace on every application process. No smoke.
set -euo pipefail
...
$PY derive_dsb_sn_nw.py --source $OPT $BASE --rates $(seq -s ' ' 1000 1000 20000) --out $B --kinds v pb cgpb sb \
  --note "SN no-work, optimized SDK (5f021f53 images), GOGC=off GOMEMLIMIT=1GiB gctrace on every application process; ramp 1000..20000 step 1000; matrix settings otherwise. No smoke." > $S/snnw_mem_opt_derive.log 2>&1
