...
R = Path(open('/tmp/claude-20002/-users-tomislav/6693a286-50fe-4f08-88f4-79a32690db39/scratchpad/nw_root').read().strip())
...
print('wrk:', w.replace('\n', ' || ') if w else 'none')
plan = json.loads((R / 'plan.json').read_text()); total_pts = len(plan['cases']) * len(plan['ramp_rates'])
total = 0; latest = None
...
print(f'measured points {total} / {total_pts}; ramps complete {sum((c/"complete.json").exists() for c in (R/"run").iterdir() if c.is_dir() and "-interrupted-" not in c.name)} / {len(plan["cases"])}')
