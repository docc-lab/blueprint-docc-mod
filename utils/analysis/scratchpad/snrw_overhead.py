...
flat = [r for r in rates if stats(data['v'], r)[1] <= 1.25 * v0]
peak = {k: max((stats(data[k], r)[0], r) for r in rates) for k in data}
knee = peak['v'][1]
print('rates', rates[0], '..', rates[-1], '| flat region (v mean within 25% of its', rates[0], 'value):', flat, '| v peak (knee) at', knee)
