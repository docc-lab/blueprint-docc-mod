...
prev = [r for r in (res(p) for p in sorted(glob.glob(os.path.dirname(os.path.dirname(f)) + '/rate-*/result.json'))) if r and r['offered_rps'] < off]
if got < 0.97 * off: line += "  << SHORTFALL"
if prev:
    best = max(prev, key=lambda r: r['completed_rps'])
    if got < 0.98 * best["completed_rps"]: line += f"  << ZIGZAG (>2% drop): below {best['offered_rps']} point ({best['completed_rps']:,.0f})"
