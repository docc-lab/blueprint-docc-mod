...
# plus the depth_cubic configdiscovery change AND the priority processor us_margin_mode=backlog + lp_refusal_code + cpu_shed_threshold options , priorityotlp pre-decode receiver, graded cpu_target LP admission, CPU window fix (defaults unchanged). Pushed under a
...
TAG=10.10.1.1:30000/otelcontribcol:prio-graded-20260923
...
echo "$DIGEST" > $S/collector_graded_digest
