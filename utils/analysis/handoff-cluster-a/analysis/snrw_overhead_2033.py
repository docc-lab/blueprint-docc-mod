...
flat = [r for r in rates if 2000 <= r <= 3300]
peak = {k: max((stats(data[k], r)[0], r) for r in rates) for k in data}
knee = peak['v'][1]
print('rates', rates[0], '..', rates[-1], '| region (fixed 2.0k-3.3k, whole range to saturation; v mean at', rates[0], 'value):', flat, '| v peak (knee) at', knee)
...
# overall request-weighted mean latency over the whole region (every request at every rate counts once), bridge vs v
def overall(reps):
    num = den = 0.0
    for r in flat:
        for rep in reps:
            if r in rep: num += rep[r][1] * rep[r][2]; den += rep[r][2]
    return num / den
ov = overall(data['v'])
for k in ('pb', 'cgpb', 'sb'):
    ob = overall(data[k]); bs = []
    for _ in range(400):
        bv = overall([data['v'][i] for i in rng.integers(0, 5, 5)]); bb = overall([data[k][i] for i in rng.integers(0, 5, 5)]); bs.append(bb / bv - 1)
    print(f'overall request-weighted mean {k.upper()}: {ob:.1f} ms vs v {ov:.1f} ms -> {100*(ob/ov-1):+.1f}% [{100*np.percentile(bs,5):+.1f},{100*np.percentile(bs,95):+.1f}]')
