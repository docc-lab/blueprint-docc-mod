...
The plugin now supports custom YAML configuration files that follow OpenTelemetry Collector conventions. This allows you to:

1. **Use any base image**: Specify custom Docker images (e.g., specific versions, custom builds)
2. **Configure processors dynamically**: Add memory limiters, samplers, attribute processors, etc.
3. **Add multiple exporters**: Configure logging, Prometheus, and other exporters
4. **Customize receivers**: Add HTTP receivers, custom protocols, etc.

### Usage with Custom Configuration

```go
// Use custom configuration file and base image
...
exporters:
  otlp:
    endpoint: "${JAEGER_OTLP_DIAL_ADDR}"
    tls:
      insecure: true
  logging:
    loglevel: debug
...
service:
  pipelines:
    traces:
      receivers: [otlp]
      processors: [memory_limiter, probabilistic_sampler, batch, attributes]
...
- **Backward Compatibility**: The original `Collector()` function continues to work as before
- **Environment Variable Support**: Custom configs can use environment variables like `${JAEGER_OTLP_DIAL_ADDR}`
- **Flexible Processors**: Add any combination of processors supported by the OpenTelemetry Collector
- **Multiple Exporters**: Configure multiple exporters for different observability backends
- **Custom Base Images**: Use specific versions or custom builds of the collector 